...
def analyze_performance(df, max_processes=None, chunk_sizes=[1, 5, 10, 20, 50]):
    ...
    for num_processes in process_counts:
        for chunk_size in chunk_sizes:
            ...
            efficiency = speedup / num_processes  # This shows how effectively we use each processor
...
def plot_performance_metrics(performance_df):
    ...
    for chunk_size in performance_df['chunk_size'].unique():
        if chunk_size != 'N/A':
            ...
            ax1.plot(df_chunk['processes'], df_chunk['time'], 
    ...
    for chunk_size in performance_df['chunk_size'].unique():
        if chunk_size != 'N/A':
            df_chunk = performance_df[performance_df['chunk_size'] == chunk_size]
            ax2.plot(df_chunk['processes'], df_chunk['speedup'], 
                    marker='o', label=f'Chunk size {chunk_size}')
    ...
    for chunk_size in performance_df['chunk_size'].unique():
        if chunk_size != 'N/A':
            df_chunk = performance_df[performance_df['chunk_size'] == chunk_size]
            ax3.plot(df_chunk['processes'], df_chunk['cpu_percent'], 
                    marker='o', label=f'Chunk size {chunk_size}')
    ...
    for chunk_size in performance_df['chunk_size'].unique():
        if chunk_size != 'N/A':
            df_chunk = performance_df[performance_df['chunk_size'] == chunk_size]
            ax4.plot(df_chunk['processes'], df_chunk['memory_used'], 
                    marker='o', label=f'Chunk size {chunk_size}')
