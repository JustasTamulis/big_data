...
# outdated
def visualize_on_map_outdated(df, anomaly_vessels, anomaly_clusters, output_file='vessel_map.html'):
    """
    ...
    sample_size = min(1000, normal_vessels.MMSI.nunique() - 1)  # Limit to 100 normal vessels for clarity
    sampled_vessels = normal_vessels.groupby('MMSI').last().sample(n=sample_size, random_state=42)
    ...
    for vessel in anomaly_vessels:
        mmsi = vessel['MMSI']
        vessel_anomalies = [a for cluster in anomaly_clusters for a in cluster if a['mmsi'] == mmsi]
    ...
    colors = ['#%06x' % random.randint(0, 0xFFFFFF) for _ in range(len(anomaly_clusters))]
